...
/**
 * Custom EditText class which simplifies setting a custom font. Font is set via custom view attributes
 */
public class BBBEditText extends EditText {
    private static final int[] STATE_ERROR = { R.attr.state_error };

    private boolean mIsError = false;

    public BBBEditText(Context context) {
        super(context);
    }

    public BBBEditText(Context context, AttributeSet attrs, int defStyle) {
    ...
    public BBBEditText(Context context, AttributeSet attrs) {
        super(context, attrs);
        if (!isInEditMode()) {
            setCustomAttributes(context, attrs);
        }
